...
CREATE POLICY "Anyone can view active crops" ON crops
    FOR SELECT USING (status = 'active' OR status = 'auction' OR auth.role() = 'service_role');

CREATE POLICY "Farmers can manage their crops" ON crops
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE users.id = crops.farmer_id 
            AND users.wallet_address = auth.uid()::text
        ) OR auth.role() = 'service_role'
    );

CREATE POLICY "Anyone can create crops" ON crops
    FOR INSERT WITH CHECK (true);
...
CREATE POLICY "Farmers can manage their auctions" ON auctions
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM crops 
            JOIN users ON users.id = crops.farmer_id
            WHERE crops.id = auctions.crop_id 
            AND users.wallet_address = auth.uid()::text
        ) OR auth.role() = 'service_role'
    );
...
CREATE POLICY "Users can view their orders" ON orders
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE (users.id = orders.buyer_id OR users.id = orders.farmer_id)
            AND users.wallet_address = auth.uid()::text
        ) OR auth.role() = 'service_role'
    );
...
CREATE POLICY "Users can view relevant offers" ON offers
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE users.id = offers.buyer_id 
            AND users.wallet_address = auth.uid()::text
        ) OR 
        EXISTS (
            SELECT 1 FROM crops 
            JOIN users ON users.id = crops.farmer_id
            WHERE crops.id = offers.crop_id 
            AND users.wallet_address = auth.uid()::text
        ) OR auth.role() = 'service_role'
    );
